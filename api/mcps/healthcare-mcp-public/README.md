...
4. Run the server:
  ...
  ### Option 5: Run directly from GitHub with `npx` (no manual clone)

  You can run the server directly from the GitHub repository without cloning it locally using `npx`. This fetches the repository package to a temporary location and runs the package's CLI.

  ```bash
  # Run the package's CLI (will download and install dependencies in a temporary cache)
  npx --yes github:Cicatriiz/healthcare-mcp-public

  # Pass args to the underlying binary (forwarded after `--`), e.g. to set a port:
  npx --yes github:Cicatriiz/healthcare-mcp-public -- --port 8000
  ```

  Notes and cautions:
  - `npx` will download the repository and install its dependencies locally (to npm cache/tmp) before running — network access is required.
  - This does not avoid downloading code; it only avoids a manual `git clone` and local install steps.
  - For private repositories you must have access (or clone/publish to a registry reachable with credentials).
  - Running remote code has security risks — inspect the repository before running or run inside an isolated environment (container).
